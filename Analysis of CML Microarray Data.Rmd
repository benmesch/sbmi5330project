...
```{r}
grade.groups <- mainz@phenoData@data[,"grade"]
grade.groups
```

```{r}
test.exprs <- mainz@assayData[["exprs"]][,which(grade.groups == 1)]# | grade.groups == 3)]
head(test.exprs)[,1:5]

```
```{r}
geom.mean(test.exprs,na.rm=TRUE)
```
...
head(x)
```
#### The enrichDO function requires an entrezgene ID vector as input, mostly is the differential gene list of gene expression profile studies. If user needs to convert other gene ID type to entrezgene ID, we recommend using bitr function provided b
